...
This roadshow event is organized to allow you to have a hands-on experience with Red Hat OpenShift Virtualization, running on Intel Xeon processors.

In this event we will explore many common management activities that virtualization administrators often encounter in their day to day workflows — all running on Intel hardware.
...
image::intel-logo.svg[Intel,120,link=self]

This lab environment runs on Intel Xeon processors, demonstrating that OpenShift Virtualization delivers the same enterprise-grade VM management on Intel hardware that organizations rely on in production.
