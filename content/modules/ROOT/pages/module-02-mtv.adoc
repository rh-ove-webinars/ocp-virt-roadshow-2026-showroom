...
NOTE: This lab environment is powered by Intel Xeon processors.
...
++++
<iframe
  src="https://demo.arcade.software/lmVpLJxDaTpk5G8vKoX1?embed&embed_mobile=tab&embed_desktop=inline&show_copy_link=true"
  width="100%"
  height="600px"
  frameborder="0"
...
+
image::2026_summer/module-02-mtv/06_MTV_Providers.png[link=self, window=blank, width=100%]
...
* winweb02-{user}
+
image::2026_summer/module-02-mtv/09_VM_List.png[link=self, window=blank, width=100%]
...
. On the next screen you will be tasked with providing storage details for your migration plan. Select the radio button to *Use new storage map*, then from the Source storage dropdowns make sure that the workload share from the VMware vSphere environment is listed, and from the Target storage dropdowns the default storage class *ocs-external-storagecluster-ceph-rbd* are selected for both. Finally, fill in the Storage map name with *default-storage-map* and click the *Next* button.
+
image::2026_summer/module-02-mtv/10b_Mig_Plan_Stor.png[link=self, window=blank, width=100%]
...
image::2026_summer/module-02-mtv/11b_Create_Migration_Plan_Bottom.png[link=self, window=blank, width=100%]
