...
* _Automating VMs with Ansible Automation Platform_: In this section we will see how Red Hat Ansible Automation Platform and OpenShift Virtualization work together, by building a dynamic inventory that discovers a virtual machine, taking a snapshot of it through the OpenShift API, and then running a pre-built playbook inside it to demonstrate automated patch management.
...
* Red Hat Ansible Automation Platform (version TBD by your workshop facilitator)
